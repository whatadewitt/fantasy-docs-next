...
const TransactionsFetch = () => {
  ...
  const formatKeys = () => transactionKey.replace(" ", "").split(",");
  ...
  return (
    <>
      ...
      <div>
        ...
        <div>
          <h3>{activeTab === "tester" ? "Code" : "How to use"}</h3>
...
try {
  const transactions = await yf.transactions.fetch(
    [${activeTab === "tester" && transactionKey !== null ? `"${formatKeys().join("\", \"")}"` : `transaction_keys` }]${(activeTab !== "tester" || Object.keys(filters).length > 0) ? "," : ""},
    ${activeTab !== "tester" ? "{filters}, // optional" : ''}
    ${Object.keys(filters).length > 0 ? `{${Object.entries(filters).map(({key, val}) => `${key}: ${val}`).join("\n,")}` : ''}
  );
} catch (e) {
  // handle error
}
