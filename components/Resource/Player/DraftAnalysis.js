...
const PlayerDraftAnalysis = () => {
  ...
  const makeAPICall = async (e) => {
    ...
    const data = await api("/player/draft_analysis", {
      playerKey,
    });
  ...
  return (
    <>
      <h2 className={appStyles.public}>player.draft_analysis</h2>
      <p>Retrieve analysis on a players draft results.</p>
...
// promise based
try {
  const draft_analysis = yf.player.draft_analysis(player_key);
} catch (e) {
  // handle error
}

// callback based
yf.player.draft_analysis(player_key, callbackFn);`}</CodeBlock>
            </div>
...
export default PlayerDraftAnalysis;
