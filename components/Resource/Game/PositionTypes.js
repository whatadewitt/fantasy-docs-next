...
const GamePositionTypes = () => {
  ...
  const makeAPICall = async (e) => {
    ...
    const data = await api("/game/position_types", {
      gameKey,
    });
  ...
  return (
    <>
      <h2 className={appStyles.public}>game.position_types</h2>
      <p>
        Retrieve a detailed description of all player position types for a
        specified game.
      </p>
...
// callback based
yf.game.position_types(game_key, callbackFn);`}</CodeBlock>
            </div>
                ...
                {JSON.stringify(
                  {
                    ...
                    season: "2014",
                    position_types: [
                      { type: "O", display_name: "Offense" },
                      { type: "K", display_name: "Kickers" },
                      { type: "DT", display_name: "Defense/Special Teams" },
                      { type: "DP", display_name: "Defensive Players" },
                    ],
                  },
                  null,
                  2
...
export default GamePositionTypes;
