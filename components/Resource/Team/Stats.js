...
const TeamStats = () => {
  ...
  return (
    <>
      <h2 className={cx(appStyles.public, appStyles.private)}>team.stats</h2>
      <p>
        Retrieve a teams statistics. Users must be authenticated and a member of
        the league to query against teams of private leagues.
      </p>
...
// promise based
try {
  const stats = yf.team.stats(team_key);
} catch (e) {
  // handle error
}

// callback based
yf.team.stats(team_key, callbackFn);`}</CodeBlock>
            </div>
