...
const TeamStandings = () => {
  ...
  const makeAPICall = async (e) => {
    ...
    const data = await api("/team/standings", {
      teamKey,
    });
  ...
  return (
    <>
      ...
      <p>
        Retrieve a teams rank, wins, losses, ties, winning percentage, etc.
        Users must be authenticated and a member of the league to query against
        teams of private leagues.
      </p>
...
// promise based
try {
  const standings = yf.team.standings(team_key);
} catch (e) {
  // handle error
}

// callback based
yf.team.standings(team_key, callbackFn);`}</CodeBlock>
            </div>
