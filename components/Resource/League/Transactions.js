...
const LeagueTransactions = () => {
  ...
  const makeAPICall = async (e) => {
    ...
    const data = await api("/league/transactions", {
      leagueKey,
    });
  ...
  return (
    <>
      ...
      <p>
        Subresource to data about league transactions, including adds, drops,
        trades, and commisionner moves. Users must be authenticated and a member
        of the league to query against private leagues.
      </p>
      <p>
        <em>
          Please note that this function does return ALL league transactions, so
          it may take some time to parse based on your league.
        </em>
      </p>
...
// promise based
try {
  const transactions = yf.league.transactions(league_key);
} catch (e) {
  // handle error
}

// callback based
yf.league.transactions(league_key, callbackFn);`}</CodeBlock>
            </div>
